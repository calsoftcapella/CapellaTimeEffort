...
<link rel="stylesheet" type="text/css"
	href="css/time_entry_page_style.css">
<link rel="stylesheet" href="css/jquery-ui.css" />
<!--  Stylesheet for Help text -->
<link rel="stylesheet" type="text/css"
...
<div style="margin-left: 40px;"><logic:present
	name="showAlertNotification" scope="request">
	<input type="hidden" id="notificationId"
		value="<bean:write name="showAlertNotification" scope="request"/>"
		style="width: 0px; height: 0px;">
</logic:present> <logic:present name="selectResourceBasedOnUserId" scope="request">
	<input type="hidden" id="selectedResourceUserId"
		value="<bean:write name="selectResourceBasedOnUserId" scope="request"/>"
		style="width: 0px; height: 0px;">
</logic:present> <html:form action="taskAction1" method="POST">
	...
	<logic:notPresent name="month" scope="request">
		<table class="sortable1" width="800px;">
			<tr>
				<td style="width: 180px; vertical-align: top;">Select Month:<html:select
					property="month" styleId="selectedMonthId"
				...
				<logic:present name="userListSelection" scope="request">
					<td
						style="padding-left: 20px; width: 295px; vertical-align: top; position: absolute;">Select
					Resource:<html:select property="userId" styleId="userId"
						onchange="onGet('taskAction1.do?method=getDetailsBasedOnResourceName')"
						style="width: 180px;font-family: verdana, arial, sans-serif;font-size: 11px;">
						<logic:notEmpty name="userListSelection" scope="request">
							<logic:iterate name="userListSelection" id="listForUser"
								scope="request">
								<option
									value="<bean:write name="listForUser" property="userId" />">
								<bean:write name="listForUser" property="userName" /></option>
							</logic:iterate>
						</logic:notEmpty>
					</html:select> <script type="text/javascript">
							      pickSelectedResourceFromDropDown();
							</script></td>
					<td style="width: 280px; vertical-align: top;"><!-- Updates for Missing Entry Table  -->
					<logic:present name="allmissingDateList" scope="request">
						<logic:notEmpty name="allmissingDateList" scope="request">
							...
							<table border="0" width="280" id="myMissingDates"
								...
								<thead>
									<tr>
										<th style="text-align: left; background-color: #C0C0C0;">Missing/Shortfall
										Entries</th>
									</tr>
								</thead>
								<tbody>
									<tr>
										<td style="text-align: left;" id="missingDateTdId"><logic:iterate
											id="missingDateId" name="allmissingDateList" scope="request"
											indexId="dConter">
											<a href="##." id="<%=dConter++ %>"
												onclick="addRowForCurrentDate(this);"
												style="font-weight: bold;"><bean:write
												name="missingDateId" /></a>
										</logic:iterate> <a href="##." onclick="addAllMissingDate(this);"><br>
										<br>
										Fill All</a></td>
									</tr>
								</tbody>
					...
					</logic:present> <!-- Updates for Missing Entry Table  --></td>
				</logic:present>
				<!-- Resource based Timesheet deatils for Raghavi.  -->
					...
					<logic:present name="allmissingDateList" scope="request">
						<logic:notEmpty name="allmissingDateList" scope="request">
							...
							<table border="0" width="280" id="myMissingDates"
								...
								<thead>
									<tr>
										<th style="text-align: left; background-color: #A5A1A0;">Missing/Shortfall
										Entries</th>
									</tr>
								</thead>
								<tbody>
	...
	<div style="text-align: center;"><img width="100%" height="100%"
		id="loading_image" src="img/loader.gif" /> <script>hideLoadingImageWhileLoading();</script>
	</div>
	<table style="padding-top: 10px;">
		<tr>
			...
			<td style="text-align: left;"><input type="button"
				...
				class="changeButtonUI" title="Click to save all new entries."
				style="font-family: verdana, arial, sans-serif; font-size: 11px; text-align: left; cursor: pointer;" />

			</td>
			<logic:present name="userListSelection" scope="request">
				<td style="text-align: left;"><input type="button"
					id="freeze_entry_id" value="Freeze Entry"
					onclick="scriptForFreezingTimesheet();" class="changeButtonUI"
	...
	<div id="myDiv" style="width: 800px; height: 800px; overflow: auto;">
	...
	<table id="dailyEntryTable" class="sortable" border="3" width="781px">
		<tr class="head">
			...
			<th style="width: 0pc">Task Desription</th>
			<th style="width: 13pc">Comments</th>
			<th>Efforts From</th>
			<th style="width: 7pc">Planned Time in Hrs</th>
			<th style="width: 7pc">Actual Time in Hrs</th>
			<th>Edit<br>
			/Save</th>
			<th>Delete</th>
		</tr>
		<logic:empty name="tList" scope="request">
			<div id="hide_table"><script type="text/javascript"> hideTableView();</script>
			</div>
		</logic:empty>
			...
			<logic:iterate id="tList" name="tList" scope="request"
				...
				<tr id="row_id<bean:write name="tList" property="id" />">
					...
					<logic:notEqual name="tList" property="status" value="Comp off">
						...
						<td style="width: 13pc"><bean:write name="tList"
							property="actualTime" /></td>
					</logic:notEqual>
					<td><img src="img/edit.png" style="cursor: pointer;"
