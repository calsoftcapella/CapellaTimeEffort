...
public class SaveTaskServiceImpl implements SaveTaskService {
	private static final Logger logger = Logger.getLogger("SaveTaskServiceImpl");
	...
	public String[] saveTask (TaskForm taskForm,int userid) throws Exception{
		...
		logger.info("Save task service impl actual time" + task.getActualTime());
		logger.info("Save task service impl time" + task.getTime());
		logger.info("Save task service impl comments" + task.getComments());
	...
	public Task populateTaskFromForm(TaskForm taskForm, int userid)throws Exception {
		...
		String actualTime = taskForm.getActualTime();
		String afterConvertActualTime=TimeConveterUtil.timeConveter(actualTime);
		task.setActualTime(afterConvertActualTime);
		return task;
	}
	...
	public TaskForm populateTaskFormFromTask(Task task)throws Exception{
		...
		taskForm.setComments(task.getComments());
		logger.info("Jaishu->checking...actual time from populateTaskFormFromTask>>>" + task.getActualTime());
		taskForm.setActualTime(task.getActualTime());
