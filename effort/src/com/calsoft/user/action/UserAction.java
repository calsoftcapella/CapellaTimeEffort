...
import com.calsoft.user.form.ProjectTaskForm;
...
import com.calsoft.user.model.UserContact;
...
public class UserAction extends DispatchAction {
	...
	public ActionForward Login(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			while (en.hasMoreElements()) {
				...
				s1.removeAttribute(sessionAttribute); // Clearing session
				// object.
			}
			...
			if (status != null && status.equalsIgnoreCase("InActive")) {
				...
				return mapping.findForward("successForAdmin");
					...
					&& (status.equals("1003") || status.equals("1006"))) {
				...
				String year = st[0];
				String month = st[1];
				List<UserForm> allocatedUserList = service
						.getAllocatedResourcesBasedOnStartAndExitDate(user_id,
								cal);
				String[] allocatedResources = new String[allocatedUserList
						.size()];
				int i = 0;
				for (UserForm userForm2 : allocatedUserList) {
					allocatedResources[i++] = Integer.toString(userForm2
							.getUserId());
				}
				reportList = reportservice.getReportData(year, month,
						allocatedResources);
				...
				for (int k = 1; k <= days; k++) {
					cal.set(Integer.parseInt(year),
							Integer.parseInt(month) - 1, k);
					dayList.add(cal.getTime().toString().substring(0, 3));
				}
				for (int j = 1; j <= days; j++) {
					cal.set(Integer.parseInt(year),
							Integer.parseInt(month) - 1, j);
				...
				request.setAttribute("dayListForDates", dayListForDates); // Setted
				// for
				// JSP
				s1.setAttribute("dayList", dayList); // Setted for excel
				s1.setAttribute("dayListForDates", dayListForDates); // Setted
				// for
				// excel
				...
				return mapping.findForward("displayReport");
					...
					&& (status != null && !status
							.equalsIgnoreCase("errorOnAddUser"))) {
				...
				if (userFormList != null && userFormList.size() > 0) {
					...
					String nextDate = format.format(format.parse(date)
							.getTime()
							+ MILLIS_IN_DAY);
					if (nextDate.equals(format.format(new Date()))) {
						request.setAttribute("errorSheet", " " + nextDate
								+ " : Enter your timesheet today.");
					}
				}
				// Added for Client feedback link.
				if (status.equals("1005") || status.equals("1007")) {
					request.setAttribute("viewClientFeedback",
							"viewClientFeedback");
				}
				// Providing link for shared feedback
				List<ClientFeedbackForm> feedbackList = service
						.getAllAddedFeedback(user_id);
				...
				if (feedbackList != null)
					for (ClientFeedbackForm clientFeedbackForm : feedbackList) {
						String completeFileName = clientFeedbackForm
								.getFile_loc().trim();
						...
						c1
								.setBody_content(clientFeedbackForm
										.getBody_content());
						try {
							...
							String folderLoc = FileSystemView
									.getFileSystemView().getHomeDirectory()
									.getAbsolutePath();
							File f1 = new File(folderLoc + File.separatorChar
									+ "client_feedback" + File.separator
									+ fileName);
							if (f1.exists()) {
								...
								logger.info("Checking folder status "
										+ !folder.isDirectory()
								...
								if (completeFileName.contains(".png")
										|| completeFileName.contains(".PNG")) {
									ImageIO.write(image, "png", new File(""
											+ folder + File.separatorChar
											+ fileName));
									c1.setFile_loc("temp_feedback_loc/"
											+ fileName);
									newFeedbackList.add(c1);
								} else if (completeFileName.contains(".jpg")
										|| completeFileName.contains(".JPG")) {
				...
				logger.info("Printing feedback list size "
						+ newFeedbackList.size());
				if (!newFeedbackList.isEmpty()) {
					request.setAttribute("feedbackList", newFeedbackList);
					request.setAttribute("feedbackListCount", newFeedbackList
							.size());
				}
				...
				return mapping.findForward("successForEmp");
					...
					&& status.equalsIgnoreCase("errorOnAddUser")) {
				ActionErrors errors = new ActionErrors();
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionError(
						"error.while.addingUser"));
				saveErrors(request, errors);
				return mapping.findForward("loginFail");
			}
			ActionErrors errors = new ActionErrors();
	...
	public ActionForward GenerateNewPassword(ActionMapping mapping,
		...
		try {
			...
			prop.load(new FileInputStream(request.getServletContext()
					.getRealPath("getInformation.properties")));
			...
			if (status == null) {
				...
				saveErrors(request, errors);
					...
					&& status.equalsIgnoreCase("Password sent.")) {
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionError(
						"email.send"));
				saveErrors(request, errors);
				...
				saveErrors(request, errors);
					...
					&& status.equalsIgnoreCase("Error while sending email.")) {
				errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionError(
	...
	public ActionForward userManage(ActionMapping mapping, ActionForm form,
		...
		if (s1.getAttribute("userName") != null) {
			...
			try {
				...
				List<ProjectDetailModel> project_id_list = service
						.getAllProjectInfo();
	...
	public ActionForward addUser(ActionMapping mapping, ActionForm form,
		...
		if (s1.getAttribute("userName") != null) {
			...
			try {
				...
				File f1 = new File(request.getServletContext().getRealPath(
				...
				if (message.equals("User added successfully")
						|| message.equals("User activated successfully")) {
					...
					if (userForm.getUserId() != 0) {
						request.setAttribute("hidden_role_id", service
								.getUserRoleId(userForm.getUserId()));
						request.setAttribute("hidden_project_id", userForm
								.getProject_id());
					}
				}
				List<UserForm> list = service.getUserNames();
	...
	public ActionForward initialProjectTaskDetails(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.info("You are inside -> method initialProjectTaskDetails");
		...
		service = Factory.getUserService();
		List<ProjectTaskForm> list = service.getProjectTaskDetails(userId);
		request.setAttribute("listOfProjectTasks", list);
		
		ProjectTaskForm projectTaskForm = new ProjectTaskForm();
		projectTaskForm.setCrmId("111");
		request.setAttribute("projectTaskForm", projectTaskForm);
		
		return mapping.findForward("initialProjectTaskDetails");
	
	}
	

	public ActionForward getUserContactDetails(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger.info("You are inside -> Class Name :: " + getClass().getName()
				+ ", -> Method Name :: " + new Object() {
				}.getClass().getEnclosingMethod().getName());
		service = Factory.getUserService();
		List<User> userContacts = service.getUserContactDetails();

		List<UserContact> chennaiContacts = new ArrayList<UserContact>();
		List<UserContact> bangaloreContacts = new ArrayList<UserContact>();
		List<UserContact> others= new ArrayList<UserContact>();

		for (User user : userContacts) {
			for (Contact contact : user.getContact()) {
				if (contact.getLocation().equalsIgnoreCase("Chennai")) {
					UserContact userContact = new UserContact();
					userContact.setUserName(user.getUser_name());
					userContact.setEmail(user.getMail());
					userContact.setCapellaEmail(user.getApollo_id());
					userContact.setSkype(contact.getSk_id());
					userContact.setMobile(contact.getContact_no());
					userContact.setLocation(contact.getLocation());
					chennaiContacts.add(userContact);
				} else if(contact.getLocation().equalsIgnoreCase("Bangalore")) {
					UserContact userContact = new UserContact();
					userContact.setUserName(user.getUser_name());
					userContact.setEmail(user.getMail());
					userContact.setCapellaEmail(user.getApollo_id());
					...
					userContact.setMobile(contact.getContact_no());
					userContact.setLocation(contact.getLocation());
					bangaloreContacts.add(userContact);
				} else {
					UserContact userContact = new UserContact();
					userContact.setUserName(user.getUser_name());
					userContact.setEmail(user.getMail());
					...
					userContact.setMobile(contact.getContact_no());
					userContact.setLocation(contact.getLocation());
					others.add(userContact);
				}
				break;
			}
		}

		request.setAttribute("chennaiContacts", chennaiContacts);
		request.setAttribute("bangaloreContacts", bangaloreContacts);
		request.setAttribute("others", others);
		return mapping.findForward("userContactDetails");
	}

	public synchronized ActionForward getHomeContent(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		try {
			...
			if (service != null && s1.getAttribute("userName") != null) {
				...
				if (resource_role_id.equals("1005")
						|| resource_role_id.equals("1007")) {
					request.setAttribute("viewClientFeedback",
							"viewClientFeedback");
				}
				// Providing link for shared feedback
	...
	public ActionForward goChangePassword(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("userName") != null) {
	...
	public ActionForward passwordChanged(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if (session1.getAttribute("userName") != null) {
			...
			try {
				service = Factory.getUserService();
				userId = Integer.parseInt(session1.getAttribute("user_id")
						.toString());
	...
	public void validateResourceExistingPassword(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		String validationStatus = userService.validateResourcePassword(
				resourceId, resourcePass);
		response.getWriter().print(validationStatus);
	}

	public ActionForward getUserAccessMapping(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("userName") != null) {
			try {
				...
				int userId = 0;
				userId = Integer
						.parseInt(s1.getAttribute("user_id").toString());
				service = Factory.getUserService();
				List<List<Report>> combinedList = service
				...
				while (allocatedIterator.hasNext()) {
					...
					if (counter == 1) {
						...
						// System.out.println("parent user name is:"+
						// report.getParentUserName());
					}
				...
				request.setAttribute("updatedallocatedList",
						updatedAllocatedList);
	...
	public ActionForward editGlobal(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("userName") != null) {
	...
	public ActionForward getAdminHome(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("userName") != null) {
	...
	public ActionForward viewClientFeedback(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("userName") != null) {
			...
			try {
				...
				List<UserForm> listForm = userService
						.getAllocatedResourcesBasedOnStartAndExitDate(admin_id,
								cal);
				String mailList = "";
	...
	public ActionForward getOpenPosition(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		return mapping.findForward("getOpenPosition");
	}

	public ActionForward shareClientFeedback(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		...
		if (s1.getAttribute("user_id") != null) {
			...
			Properties prop = new Properties();
			File admin_info_file = new File(getServlet().getServletContext()
					.getRealPath("/")
					+ "getInformation.properties");
			...
			UserService userService = Factory.getUserService();
			Calendar cal = Calendar.getInstance();
			String filePath = FileSystemView.getFileSystemView()
					.getHomeDirectory()
					+ File.separator + "client_feedback";
			String homeLoc = FileSystemView.getFileSystemView()
					.getHomeDirectory().getAbsolutePath();

			// //////////////// Checking values here ////////////////////
			FormFile fileAttached = (FormFile) PropertyUtils.getSimpleProperty(
					form, "uploded_file");
			logger.info("Printing file detail PropertyUtils " + fileAttached
					+ " from request " + request.getContentType()
					+ " encoding " + request.getCharacterEncoding());
			ClientFeedbackForm cf1 = (ClientFeedbackForm) request
					.getAttribute("clientFeedbackForm");
			logger.info("Getting image file from request object "
					+ cf1.getUploded_file());
			logger.info("Printing uploaded file existance " + file
					+ " filePath:: " + filePath + " homeLoc:: " + homeLoc);
			// //////////////// Checking values here ////////////////////
			...
			try {
				UserForm userFormForUsername = userService
						.getUsernameFromId(userId);
				if (file == null) {
					request.setAttribute("message_for_uploading",
					...
					if (!newFile.exists()) {
						...
						fos.write(file.getFileData()); // Saving uploaded file
						// to local.
						fos.flush();
						fos.close();
						String messageStatus = "Failed to send mail please try again later.";
						if (!prop.isEmpty() && toListArray != null) {
							messageStatus = service
									.setNotificationForSharedFeedback(
											toListArray, newFile,
											userFormForUsername,
											clickFeedbackForm, homeLoc, prop);
						}
						request.setAttribute("message_for_uploading",
								messageStatus);
						...
						String fileNameWithoutExtension = completeFileName
								.substring(0, completeFileName.lastIndexOf('.'));
						String fileExtension = completeFileName.substring(
	...
	public void setViewStatus(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		if (s1.getAttribute("user_id") != null) {
			...
			if (jsonString != null) {
				...
				for (int i = 0; i < object.size(); i++) {
					...
					try {
						idList.add(Integer.parseInt(jObj.get("feedbackId")
								.toString()));
					} catch (Exception e) {
						logger
								.info("Printing message while parsing feedbackId for setViewStatus::: "
										+ e.getMessage());
					}
				}
			}
			int user_id = Integer.parseInt(s1.getAttribute("user_id")
					.toString());
			service.setViewStatus(idList, user_id);
		} else
			response.getWriter().print(
					"Session Expired relogin to view shared comment.");
	}

	public ActionForward getSelectedResourceDetailForUpdate(
			ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger
				.info("Printing from user action getSelectedResourceDetailForUpdate method");
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("user_id") != null
				&& request.getParameter("selectedUserId") != null) {
			int user_id = Integer.parseInt(request
					.getParameter("selectedUserId"));
			service = Factory.getUserService();
	...
	public ActionForward saveResourceInformationDetail(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		logger
				.info("Printing from user action saveResourceInformationDetail method");
		HttpSession s1 = request.getSession();
		if (s1.getAttribute("user_id") != null) {
			UserForm user_form = (UserForm) form;
			service = Factory.getUserService();
			String status = "Error while updating resource detail.";
			logger.info("Printing from saveResourceInformationDetail "
					+ user_form.getProject_id());
			try {
				status = service.saveResourceInProfile(user_form);
				request.setAttribute("UpdateStatus", status);
				UserForm userInfoDetails = service
						.getUserDetailForUserId(user_form.getUserId());
				List<ProjectDetailModel> project_id_list = service
						.getAllProjectInfo();
				userInfoDetails.setProject_id_list(project_id_list);
	...
	/*
	 * public void getCurrentStatusForSharedComment(ActionMapping mapping,
	 * ActionForm form,HttpServletRequest request, HttpServletResponse
	 * response)throws Exception{logger.info(
	 * "Printing from user action getCurrentStatusForSharedComment method");
	 * HttpSession s1=request.getSession();
	 * if(s1.getAttribute("user_id")!=null){ //Providing link for shared
	 * feedback service = Factory.getUserService(); Integer user_id =
	 * Integer.parseInt(s1.getAttribute("user_id").toString());
	 * List<ClientFeedbackForm> feedbackList =
	 * service.getAllAddedFeedback(user_id); List<ClientFeedbackForm>
	 * newFeedbackList = new ArrayList<ClientFeedbackForm>(); if(feedbackList !=
	 * null) for (ClientFeedbackForm clientFeedbackForm : feedbackList) { String
	 * completeFileName = clientFeedbackForm.getBody_content().trim();
	 * ClientFeedbackForm c1 = new ClientFeedbackForm();
	 * c1.setUsername(clientFeedbackForm.getUsername());
	 * c1.setFeedbackId(clientFeedbackForm.getFeedbackId()); try{
	 * if(completeFileName.contains(".png") ||
	 * completeFileName.contains(".jpg")){ String fileName =
	 * completeFileName.substring
	 * (completeFileName.lastIndexOf(File.separatorChar)+1,
	 * completeFileName.length()); String folderLoc =
	 * FileSystemView.getFileSystemView().getHomeDirectory().getAbsolutePath();
	 * File f1 = new
	 * File(folderLoc+File.separatorChar+"client_feedback"+File.separator
	 * +fileName); if(f1.exists()){ String filePath =
	 * getServlet().getServletContext().getRealPath("/") +"temp_feedback_loc";
	 * //create the upload folder if not exists File folder = new
	 * File(filePath); if(!folder.isDirectory()){ folder.mkdirs(); }
	 * BufferedImage image = ImageIO.read(f1);
	 * if(completeFileName.contains(".png")){ ImageIO.write(image, "png",new
	 * File(""+folder+File.separatorChar+fileName));
	 * c1.setBody_content("temp_feedback_loc/"+fileName);
	 * newFeedbackList.add(c1); } else if(completeFileName.contains(".jpg")){
	 * ImageIO.write(image, "jpg",new
	 * File(""+folder+File.separatorChar+fileName));
	 * c1.setBody_content("temp_feedback_loc/"+fileName);
	 * newFeedbackList.add(c1); } } } else{
	 * c1.setBody_content(completeFileName); newFeedbackList.add(c1); } } catch
	 * (Exception e) { logger.info("Error while displaying shared image"); } }
	 * logger.info("Printing feedback list size "+newFeedbackList.size());
	 * if(!newFeedbackList.isEmpty()){ request.setAttribute("feedbackList",
	 * newFeedbackList); request.setAttribute("feedbackListCount",
	 * newFeedbackList.size()); }
	 * response.getWriter().print(newFeedbackList.size()); } else
	 * response.getWriter
	 * ().print("Session Expired relogin to view shared comment."); }
	 */
	// Cron Job Scheduler
	/*
	 * public void execute(JobExecutionContext context)throws
	 * JobExecutionException { service = Factory.getUserService(); try {
	 * List<UserForm> allUser = service.getUserNamesMappedUnderAccountManager();
	 * service.sendEmailToAll(allUser); } catch (Exception e) {
	 * e.printStackTrace(); } }
	 */
