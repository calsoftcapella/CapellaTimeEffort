...
public class UserDaoImpl implements UserDao{
	...
	@Override
	public List<User> getUserContactDetails() {
		Session session = HbnUtil.getSession();
		session.beginTransaction();
		List<User> users = session.createQuery("From User u left join fetch u.contact c").list();
		return users;
	}
	@Override
	public List<ProjectTaskDetails> getProjectTaskDetails(int userId) {
		Session session = HbnUtil.getSession();
		session.beginTransaction();
		
		List<ProjectTaskDetails> list = (List<ProjectTaskDetails>) session.createQuery("From ProjectTaskDetails where user.user_Id=" + userId + " order by crmId" ).list();
		return list;
	}
}
