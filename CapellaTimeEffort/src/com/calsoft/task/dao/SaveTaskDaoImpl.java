...
import com.calsoft.task.model.ProjectTaskDetails;
...
public class SaveTaskDaoImpl implements SaveTaskDao {
	...
	public List<Task> getTaskDetails(String month,int userId) throws Exception {
		...
		String sqlQuery ="select id,backlog_id,task_id,task_description,ROUND(time/10000,2) as time,task_date,status,user_id,work_status,ROUND(actual_time/10000,2) as actual_time,comments from employee_task_detail where user_id="+userId+" and  task_date like '"+ month+"%' ORDER BY task_date DESC";
		...
		while (itr.hasNext()) {
			...
			Double actualTime = (Double) obj[9];
			String actualTimeString = actualTime.toString();
			logger.info("Jaishu->checking...actual time>>>" + actualTimeString);
			String comments = (String) obj[10];
			task.setId(id);
			...
			task.setActualTime(actualTimeString);
			logger.info("Jaishu->checking...actual time from task>>>" + task.getActualTime());
		...
		try{			
			...
			if(list!=null && list.get(0)!=null){
				...
				if(total>23){
					...
					tk.setComments(task.getComments());
					tk.setActualTime(task.getActualTime());
					session.update(tk);
